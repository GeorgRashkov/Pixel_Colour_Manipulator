...
class Mask():
    ...
    def apply_regions(self, img_for_colour_regions:np.ndarray[np.uint8], img_for_colour_range_regions:np.ndarray[np.uint8]):
        ...
        if(img_for_colour_regions.shape[0] > 0 and img_for_colour_regions.shape[1] > 0):
            self.rgb_formulas_mask.create_colour_regions(img_for_creating_a_mask=img_for_colour_regions, colours=self.colours, remove_previous_mask=self.remove_previous_mask_when_applying_mask)
            if(img_for_colour_range_regions.shape[0] > 0 and img_for_colour_range_regions.shape[1] > 0):
                self.rgb_formulas_mask.create_colour_range_regions(img_for_creating_a_mask=img_for_colour_range_regions, colour_ranges=self.colour_ranges, remove_previous_mask=False)
        
        elif(img_for_colour_range_regions.shape[0] > 0 and img_for_colour_range_regions.shape[1] > 0):
            self.rgb_formulas_mask.create_colour_range_regions(img_for_creating_a_mask=img_for_colour_range_regions, colour_ranges=self.colour_ranges, remove_previous_mask=self.remove_previous_mask_when_applying_mask)
